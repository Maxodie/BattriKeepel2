...
public class SpiralAttack : BossAttackParent {
    [SerializeField] SO_SpiralAttackData data;
    Transform m_parent;
    ...
    public void Start() {
        ...
        for (int i = 0; i < data.m_amountPerWave; i++)
        {
            position = (Vector2)m_parent.position + direction;
            m_bullets.Add(new Bullet(data.m_bulletData, position, m_parent, true, Vector3.down, typeof(GameEntity.Player)));
